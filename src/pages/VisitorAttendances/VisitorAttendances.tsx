...
import { searchValue as SearchValueType } from 'types/search';
...
const VisitorAttendances = () => {
  ...
  const { control, watch } = useForm({
    defaultValues: {
      userType: undefined as undefined | string,
      date: {
  ...
  const searchValue: SearchValueType = paramsStrToObj(location.search);
  ...
  useEventsSocket({
    ...
    onActionCreated: (newAction) => {
      ...
      setMergedData((prev: any) => {
        ...
        if (newAction?.id && list.some((item: any) => item?.id === newAction.id)) {
          return prev;
        }
            ...
            <Controller
              ...
              render={({ field }) => (
                <MySelect
                  ...
                  value={userTypeOptions.find((opt) => opt.value === (field.value ?? '')) ?? undefined}
                  onChange={(val: any) =>
                    field.onChange(
