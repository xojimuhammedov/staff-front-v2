...
export const createColumns = () => {
    ...
    const renderEntryType = (type?: string) => {
        ...
        return (
            <span
                className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold ${isEnter
            ...
            >
                <span className="flex h-5 w-5 items-center justify-center">
                    {isEnter ? (
                        <LogIn className="h-3.5 w-3.5 text-green-600 dark:text-green-300" />
                    ) : (
                        <LogOut className="h-3.5 w-3.5 text-orange-600 dark:text-orange-300" />
                    )}
                </span>
                {t(type)}
            </span>
