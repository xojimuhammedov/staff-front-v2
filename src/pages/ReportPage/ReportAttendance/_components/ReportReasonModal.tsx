...
import { useReportFormAttendance } from '../hooks/useReportFormAttendance';

function ReportReasonModal({ row }: any) {
  const { t, i18n } = useTranslation();
  const currentLang = i18n.resolvedLanguage;
  const { open, setOpen, reasonData, register, watch, onSubmit, handleSubmit, control } =
    useReportFormAttendance({ row });
...
export default React.memo(ReportReasonModal);
