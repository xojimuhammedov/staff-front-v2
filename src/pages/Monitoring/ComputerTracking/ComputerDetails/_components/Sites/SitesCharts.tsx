        ...
        return (
            <div className="bg-white dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 p-3 rounded-lg shadow-sm">
                ...
                {payload.map((entry: any, index: number) => {
                    const labelName = entry.name === 'time' ? t('Time') : entry.name === 'visits' ? t('Visits') : entry.name;
                    const valueFormatted = entry.name === 'time' ? `${entry.value} ${t('minutes')}` : entry.name === 'visits' ? `${entry.value} ${t('times')}` : entry.value;
                    return (
...
const SitesCharts = ({ user }: { user?: any }) => {
    ...
    const statusPieData = useMemo(() => {
        return [
            { name: t('Useful'), value: productivityData?.usefulTime || 0, color: '#10B981' }, 
            { name: t('Unuseful'), value: productivityData?.unusefulTime || 0, color: '#EF4444' }, 
            { name: t('Other'), value: productivityData?.otherTime || 0, color: '#9CA3AF' }, 
        ].filter(d => d.value > 0);
    }, [productivityData, t]);
    ...
    return (
        <div className="space-y-6 mb-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="rounded-md border border-gray-200 dark:border-neutral-700 bg-white dark:bg-[#1a1c23] shadow-sm overflow-hidden lg:col-span-2">
                    <div className="p-5 pb-2 border-b border-gray-100 dark:border-neutral-800">
                        <h3 className="text-base font-semibold text-gray-900 dark:text-white">{t('Most visited sites (in minutes)')}</h3>
                    </div>
                    <div className="p-5">
                        <div className="h-64">
            ...
            <div className="rounded-md border border-gray-200 dark:border-neutral-700 bg-white dark:bg-[#1a1c23] shadow-sm overflow-hidden">
                <div className="p-5 pb-2 border-b border-gray-100 dark:border-neutral-800">
                    <h3 className="text-base font-semibold text-gray-900 dark:text-white">{t('Kunlik Tashriflar (soat boʻyicha)')}</h3>
                </div>
                <div className="p-5">
                    <div className="h-64">
                        <ResponsiveContainer width="100%" height="100%">
