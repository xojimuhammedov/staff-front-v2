        ...
        <div
          ...
          className="absolute right-2 top-14 flex w-[120px] w-[217px] cursor-pointer flex-col justify-center rounded-lg  bg-white px-4 py-3 shadow-md dark:bg-bg-dark-bg">
            <div className='bg-[#F9FAFB] p-2'>
              ...
              <p className="text-subtle font-inter text-sm">{userData?.role}</p>
            </div>
          <Link to={'/profile/edit'} className="flex items-center gap-2 pt-6">
            <img src={EditIcon} />
            <h3 className="text-sm font-medium">{t('Edit profile')}</h3>
          </Link>
          <span onClick={() => setOpenAlert(true)} className="flex items-center gap-2 pt-4">
